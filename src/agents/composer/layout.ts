...
export function buildTimeline(inputs: ComposerInputs, o: BuildOptions): MasterTimeline {
  ...
  const scenes: SceneLayout[] = inputs.script.beats.map((beat, i) => {
    ...
    if (presenterOn && pb && o.presenterSrc) {
      ...
      layers.push({ id: `presenter_${beat.id}`, type: "presenter", z: Z.presenter, start: m.start, end: m.end, rect: presenterRect, src: o.presenterSrc.src, alpha: o.presenterSrc.alpha, src_width: o.presenterSrc.width, src_height: o.presenterSrc.height, focal: pb.camera === "close" ? "50% 15%" : "50% 22%", scale_boost: pb.camera === "close" ? 1.25 : pb.camera === "medium_close" ? 1.1 : 1, feather_px: o.presenterSrc.alpha ? 0 : 48, vignette: !o.presenterSrc.alpha, transition: prevOn ? "none" : "slide_up" });
